...
create_keys() {
    ...
    prompt_master_key
    printf '%s\n' "$MASTER_KEY" | remote 'read -r MASTER_KEY; set -e; test -n "$(pgrep -u "$(id -u)" -f "[l]itellm.*--port '"$REMOTE_LITELLM_PORT"'" || true)" || { echo "LiteLLM is not running on port '"$REMOTE_LITELLM_PORT"'. Run start first." >&2; exit 1; }; grep -q "^  database_url:" "$HOME/rag_workshop/litellm_config.yaml" 2>/dev/null || { echo "No database configured (see litellm_config.yaml) -- create-keys needs a Postgres-backed LiteLLM. Run '"'"'install-db'"'"', then '"'"'start'"'"' again with the DB password." >&2; exit 1; }; umask 077; output="$HOME/rag_workshop/participant-keys.tsv"; : > "$output"; for i in $(seq 1 '"$count"'); do alias="'"$prefix"'-$(printf "%02d" "$i")"; response=$(curl -fsS -X POST http://127.0.0.1:'"$REMOTE_LITELLM_PORT"'/key/generate -H "Authorization: Bearer $MASTER_KEY" -H "Content-Type: application/json" -d "{\"duration\":\"8h\",\"max_budget\":5,\"models\":[\"'"$LITELLM_MODEL_NAME"'\"],\"key_alias\":\"$alias\"}"); key=$(printf "%s" "$response" | python3 -c "import json,sys; print(json.load(sys.stdin).get(\"key\", \"\"))"); test "${key#sk-}" != "$key" || { echo "Key generation failed for $alias" >&2; exit 1; }; printf "%s\t%s\n" "$alias" "$key" >> "$output"; done; chmod 600 "$output"; echo "Generated '"$count"' keys: $output"'
}
